...
Run on a real quantum hardware.
...
# Using a real device. Part 1 of 2. # mod_real
import time
from qiskit import IBMQ, transpile
from qiskit.tools.monitor import job_monitor
#IBMQ.save_account("YOUR TOKEN",overwrite=True) #YOUR TOKEN
...
def mapping(param, raw_data, starting_index = 0, num_data = 3,
    ...
    def FEATURE_MAP(data):
        ...
        #for i in range(2 ** num_qubits - 784): desired_state.append(0.0) #mod_8d
    ...
    # Using a real device. Part 2 of 2. # mod_real
    transpiled_circuits = transpile(circuit_qc, backend_real)
    job = backend_real.run(transpiled_circuits, shots = shots * (num_param * 2 + 1))
    #job_monitor(job)
    result = job.result()
...
num_qubits = 3
repetitions = 1 #Adjustable parameter
shots = 500 #Adjustable parameter
num_data = 20 #Adjustable parameter
...
for i in range(num_qubits * (repetitions + 1)):
...
with open('data/other_results_impr.csv','w', newline='') as resultsave: #mod_3a
    ...
    writer.writerow(('num_qubits:', num_qubits)) #mod_8d1r
    writer.writerow(('repetitions in the RealAmplitudes ansatz:', repetitions)) #mod_8d1r
    writer.writerow(('num_data:', num_data)) #mod_8d1r
